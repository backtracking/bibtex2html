...
\item a test of existence of a field, written as $\verb|exists | f$ where $f$
  is a field name ;
\end{itemize}
...
\item the special character \verb|$| which corresponds to the key of
  an entry.
\item the special character \verb|#| which corresponds to the type of
  an entry (Article, InProceedings, etc.)
\end{itemize} 
...
\mid \verb|not | Cond \mid \verb|( | Cond \verb| )| \\
&& \mid Expr ~ Comp ~ Expr \mid Expr \verb|:| String \mid \verb|exists | Id\\
Comp & \rightarrow & \verb|=| \mid \verb|>| \mid \verb|<| \mid
\verb|>=| \mid \verb|<=| \mid \verb|<>| \\
Expr & \rightarrow & Id \mid String \mid Int \mid \verb|$| \mid \verb|#| \\
Id & \rightarrow & [\verb|a|-\verb|z|\verb|A|-\verb|Z|]^+ \\
String & \rightarrow & \verb|"| [\verb|^"|]^* \verb|"| \mid \verb|'| [\verb|^'|]^* \verb|'| \\
...
\item On the other hand, case matters when comparing strings. If you want for
  ...
  value in uppercase, as for example \verb|# = "INPROCEEDINGS"|, otherwise the
  condition would be always false. 
