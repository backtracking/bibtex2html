...
\item[\texttt{-s} \textit{field}] ~

  sorts the entries of the bibliography with respect to the given
  \textit{field}, which may be \texttt{\$key} or \texttt{\$type} to
  refer to the key or to the entry type, as for filter conditions.
  This options may be used several times to specifiy a lexicographic
  order, such as by year, then by keys.
...
\item Comparison using \verb|>|, \verb|<|, \verb|>=| and \verb|<=| may only be
  ...
  the result is false.
\item There is a special handling for strings containing LaTeX accented
  letters: each variant of writing such letters are considered
  the same, and equivalent to their ISO-Latin1 form (if it exists),
  for example strings \verb|"Filli\^atre"|, \verb|"Filli{\^a}tre"|, 
  \verb|"Filli\^{a}tre"| and \verb|"Filliâtre"| are considered
